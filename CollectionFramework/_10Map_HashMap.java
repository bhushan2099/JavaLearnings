package com.CollectionsFramework;

import java.util.HashMap;
import java.util.Map;

public class _10Map_HashMap {

    // MAP is used to store key-value pairs whereas List, Set and Queue are used just to store values.

    // Keys and Values can have any datatype like Key can be an Integer while Value can be String or any custom datatype.

    // Keys are always unique in Map, Values can be duplicate

    public static void main(String[] args) {

        Map<String, Integer> numbers = new HashMap<>();

        numbers.put("One",1);
        numbers.put("Two",2);
        numbers.put("Three",3);
        numbers.put("Four",4);

//        numbers.put("Two",22);    // this will get overridden

        // Hence use 'if condition' to put values using key
        if(!numbers.containsKey("Two")){
            numbers.put("Two",22);
        }

        numbers.putIfAbsent("Two",22);   // works same as 'if condition' mentioned above

        System.out.println(numbers.containsValue(22));

        System.out.println(numbers);      // Map is unordered hence printed randomly
        System.out.println(numbers.get("One"));
        System.out.println(numbers.get("Two"));

        // to iterate in a Map following method is used -
        for(Map.Entry<String, Integer> e: numbers.entrySet()){
            System.out.println(e);
            System.out.println(e.getKey());
            System.out.println(e.getValue());
        }

        System.out.println(numbers.keySet());
        System.out.println(numbers.values());

        // Other way to iterate the Map -
        for(String key:numbers.keySet()){
            System.out.println(numbers.get(key));
        }

        numbers.remove("Three");
        System.out.println(numbers);




    }
}
